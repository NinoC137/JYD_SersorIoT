...
void GUI_sysInfoUpdate(){
  ...
  char sysTime[16];
  snprintf(sysTime, 16, "%lu ms", ProjectData.runTime);

  lv_textarea_set_text(guider_ui.screen_WiFi_SSID, WiFi.SSID().c_str());
  ...
  lv_textarea_set_text(guider_ui.screen_Dev, modbusError == CRC_ERROR ? "connect!" : (const char *)me);
